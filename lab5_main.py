...
def MoveBlock(pub_cmd, loop_rate, start_xy, target_xy, vel, accel):
    ...
    start_up = inverse_kinematics(start_xy[0], start_xy[1], z_upper, 0)
    start_down = inverse_kinematics(start_xy[0], start_xy[1], z_lower, 0)
    dest_up = inverse_kinematics(target_xy[0], target_xy[1], z_upper, 0)
    dest_down = inverse_kinematics(target_xy[0], target_xy[1], z_lower, 0)

    move_arm(pub_cmd, loop_rate, start_up, vel, accel)
    move_arm(pub_cmd, loop_rate, start_down, vel, accel)
    gripper(pub_cmd, loop_rate, True)
    move_arm(pub_cmd, loop_rate, start_up, vel, accel)

    move_arm(pub_cmd, loop_rate, dest_up, vel, accel)
    move_arm(pub_cmd, loop_rate, dest_down, vel, accel)
...
def main():
    ...
    start_red = FindColorBlockWorldCoord(current_img, "RED")
    start_green = FindColorBlockWorldCoord(current_img, "GREEN")

    MoveBlock(pub_command, loop_rate, start_red, dest_block_xy[0], vel, accel)
    MoveBlock(pub_command, loop_rate, start_yellow, dest_block_xy[1], vel, accel)
